...
class SysNoticeControllerTest {
    ...
    void testList_Success() throws Exception {
        ...
        when(noticeService.selectNoticeList(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(notices);
    ...
    void testList_Empty() throws Exception {
        // Arrange
        when(noticeService.selectNoticeList(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(new ArrayList<>());
    ...
    void testAddSave_NoticeType_Success() throws Exception {
        // Arrange
        when(noticeService.insertNotice(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(1);
    ...
    void testAddSave_InsertFailed() throws Exception {
        // Arrange
        when(noticeService.insertNotice(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(0);
    ...
    void testEditSave_Success() throws Exception {
        // Arrange
        when(noticeService.updateNotice(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(1);
    ...
    void testEditSave_UpdateFailed() throws Exception {
        // Arrange
        when(noticeService.updateNotice(org.mockito.ArgumentMatchers.any(SysNotice.class))).thenReturn(0);
    ...
    @Test
    @Order(40)
    @DisplayName("边界条件-公告标题包含特殊字符-XSS过滤生效")
    void testAddSave_SpecialCharacters() throws Exception {
        // XSS过滤应该生效，返回400表示校验失败
        // 这证明系统的安全机制正常工作，不需要mock service
        mockMvc.perform(post("/system/notice/add")
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .param("noticeTitle", "<script>alert('xss')</script>")
                        .param("noticeType", "1")
                        .param("noticeContent", "内容"))
                .andExpect(status().isBadRequest()); // XSS过滤生效，返回400
    }
